...
## Dapa prep
data <- read_excel('Dapagliflozin.xlsx', sheet = 5, skip = 1)
"%ni%" <- Negate("%in%")
...
data <- read_excel('Metformin.xlsx', sheet = 5, skip = 1)
"%ni%" <- Negate("%in%")
...
## NCA calculation
metformin_NCA <- tblNCA(metformin, key = c("Period", "ID"), colTime = "Time", colConc = "Conc", dose = 1000, adm = "Extravascular", R2ADJ = -1, concUnit = 'ng/mL') %>%
  select(Period, ID, CMAX, TMAX, LAMZHL, AUCLST, AUCIFO, CLFO, VZFO) 
...
## Comparative PK
metformin_BE_raw <- metformin_NCA  %>%
  mutate(LCMAX = log10(CMAX), LAUCLST = log10(AUCLST))

f <- LCMAX ~ Period  # LCMAX, LAUCLST
# f <- LAUCLST ~ Period 

BE <- lme(f, random = ~1|ID, data = metformin_BE_raw)    
ci <- intervals(BE, 0.9)
exp(ci$fixed["Period2기", ])    ## 90% CI result 

GLM(f, metformin_BE_raw)$ANOVA     ## Anova result
...
data <- read_excel('Metformin.xlsx', sheet = 6, skip = 1)
"%ni%" <- Negate("%in%")
...
data <- read_excel('Valsartan.xlsx', sheet = 5, skip = 1)
"%ni%" <- Negate("%in%")
...
## NCA calculation
valsartan_NCA <- tblNCA(valsartan, key = c("Period", "ID"), colTime = "Time", colConc = "Conc", dose = 160, adm = "Extravascular", R2ADJ = -1, concUnit = 'ng/mL') %>%
  select(Period, ID, CMAX, TMAX, LAMZHL, AUCLST, AUCIFO, CLFO, VZFO) 
...
## Comparative PK
valsartan_BE_raw <- valsartan_NCA  %>%
  mutate(LCMAX = log10(CMAX), LAUCLST = log10(AUCLST))

f <- LCMAX ~ Period  # LCMAX, LAUCLST
# f <- LAUCLST ~ Period 

BE <- lme(f, random = ~1|ID, data = valsartan_BE_raw)    
ci <- intervals(BE, 0.9)
exp(ci$fixed["Period2기", ])    ## 90% CI result 

GLM(f, valsartan_BE_raw)$ANOVA     ## Anova result
...
data <- read_excel('Valsartan.xlsx', sheet = 6, skip = 1)
"%ni%" <- Negate("%in%")
